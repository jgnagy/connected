...
RSpec.describe Connected::Path do
  ...
  subject do
    Connected::Path.all(from: nodes['a'], to: nodes['e'])
  end

  let(:graph_with_suboptimal_connections) do
    Connected::Path.all(from: nodes['a'], to: nodes['e'], suboptimal: true)
  end

  let(:graph_with_closed_connection) do
    Connected::Path.all(from: nodes['a'], to: nodes['f'])
  end

  let(:graph_with_allowed_closed_connection) do
    Connected::Path.all(from: nodes['a'], to: nodes['f'], include_closed: true)
  end

  let(:lowest_cost) do
    Connected::Path.find(from: nodes['a'], to: nodes['e'])
  end

  let(:lowest_directed_cost) do
    Connected::Path.find(from: directed_nodes['a'], to: directed_nodes['e'])
  end
  ...
  it 'finds all paths including suboptimal paths' do
    expect(graph_with_suboptimal_connections).not_to be nil
